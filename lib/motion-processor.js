 ...
 *
 *   1. temaPredito — the BERTimbau classifier's output, ranked by confidence
...
function pickTheme(motion) {
  const bertimbau = motion?.temaPredito;
  if (Array.isArray(bertimbau)) {
    if (bertimbau.length && String(bertimbau[0]).trim()) return String(bertimbau[0]).trim();
  } else if (bertimbau !== undefined && bertimbau !== null && String(bertimbau).trim()) {
    return String(bertimbau).split(';')[0].trim();
  }
 ...
 * when there is only one). The entry with Sigla "GOV." is the government's
 * declared position — it makes "did the government's side prevail?" a direct
 * comparison instead of an inference from vote tallies.
 *
 * Siglas are kept exactly as published (blocs like "PsbPtbPcdob" and "Minoria"
 * are official records, not party names to normalize). Coverage is only
 * consistent from ~1999 on; older roll calls simply have no block.
 *
...
function extractOrientations(votacao) {
  ...
  for (const bancada of bancadas) {
    ...
    if (sigla) orientations[sigla] = orientacao;
  }
